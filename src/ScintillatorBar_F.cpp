...
ScintillatorBar_F::ScintillatorBar_F(unsigned int bIndex)
    : fBarIndex(bIndex), fQlong(0), fTstamp(0), fTime(0), fDelt(0), fQnear(0),
      fQfar(0) {}
ScintillatorBar_F::ScintillatorBar_F(ULong64_t evNo, ushort barIndex,
                                     UInt_t qlong, ULong64_t tstamp,
                                     UInt_t wtime, Int_t deltstamp)
    : fEvNo(evNo), fBarIndex(barIndex / 2), fQlong(qlong), fTstamp(tstamp),
      fTime(wtime), fDelt(deltstamp) {
...
ScintillatorBar_F::ScintillatorBar_F(ushort barIndex, UInt_t qlong,
                                     ULong64_t tstamp, UInt_t wtime,
                                     Int_t deltstamp)
    : fBarIndex(barIndex / 2), fQlong(qlong), fTstamp(tstamp), fTime(wtime),
      fDelt(deltstamp) {
  unsigned short int maxU_16bits = USHRT_MAX;
  UInt_t maskingVal = maxU_16bits;
  fQfar = (fQlong & maskingVal);
...
ScintillatorBar_F::ScintillatorBar_F(ushort barIndex, UInt_t qnear, UInt_t qfar,
                                     ULong64_t tstamp, UInt_t wtime,
                                     Int_t deltstamp)
    : fBarIndex(barIndex / 2), fQnear(qnear), fQfar(qfar), fTstamp(tstamp),
      fTime(wtime), fDelt(deltstamp) {
#ifdef DEB
  fBrCh = barIndex;
#endif
...
#endif
  std::cout << "BarIndex : " << fBarIndex
            << " : LayerIndex : " << GetLayerIndex()
            << " : BAR Name : " << vecOfPsBars[fBarIndex]
            << " :  Energy :  " << GetQMean()
            << ": Corrected Energy : " << GetQMeanCorrected()
            << " : DelT : " << GetDelT() << " : TNear : " << GetTStampNear()
            << " : TFar : " << GetTStampFar()
            << " : TSmall : " << GetTStampSmall() << std::endl;
}

ScintillatorBar_F::~ScintillatorBar_F() {}

ushort ScintillatorBar_F::GetBarIndex() const { return fBarIndex; }
...
Double_t ScintillatorBar_F::GetQMean() { return sqrt(GetQNear() * GetQFar()); }
/*Double_t ScintillatorBar_F::GetQMeanCorrected()
{
  //  return GetQMean();
  return (20. / (1. * GetPeakPos(vecOfBarsNamess[fBarIndex]))) * GetQMean();
}*/
Long_t ScintillatorBar_F::GetDelT() const { return fDelt; }

Long_t ScintillatorBar_F::GetTStampNear() const { return fTstamp; }

Long_t ScintillatorBar_F::GetTStampFar() const { return (fTstamp - fDelt); }

Long_t ScintillatorBar_F::GetTStampSmall() const {
  if (fDelt < 0)
...
Long_t ScintillatorBar_F::GetTStampLarge() const {
  if (fDelt < 0)
    return GetTStampFar();
  else
    return GetTStampNear();
...
Long_t ScintillatorBar_F::GetTStampAverage() const {
  return (GetTStampNear() - GetTStampFar()) / 2.0 + GetTStampFar() -
         1.66782048E-9;
}
...
double ScintillatorBar_F::GetOffsetCorrection() {
  ...
  unsigned int sequentialBarIndex =
  GetIndexFromBarName(sequentialBarNames,barName); std::cout << RED << "BARName
  : " << barName << std::endl; std::cout << RED << "Sequential Bar Index : " <<
  sequentialBarIndex << RESET << std::endl; return
  Calibration::instance()->GetCalibrationDataOf(sequentialBarIndex)->GetDelTOffset()
  * 1000;
  */

  return Calibration::instance()
             ->GetCalibrationDataOf(fBarIndex)
             ->GetDelTOffset() *
         1000;
  // return 0;
}
...
TF1 *ScintillatorBar_F::GetZParameterization() {
  ...
  unsigned int sequentialBarIndex = GetIndexFromBarName(barName);
  return Calibration::instance()
      ->GetCalibrationDataOf(sequentialBarIndex)
      ->GetZParamFormula();
}
...
Double_t ScintillatorBar_F::GetQMeanCorrected(unsigned int muonPeakPos) {
  ...
  TF1 *enercalibFormula = Calibration::instance()
                              ->GetCalibrationDataOf(sequentialBarIndex)
                              ->GetEnergyCalibFormula();
  ener = (enercalibFormula->Eval(GetQMean()));
  // std::cout << "Predicted Energy : " << ener << std::endl;
#else
  // Using single point calibration, and using equation of straight line to get
  // y correspoinding to a x
...
Calibration::instance()->GetCalibrationDataOf(sequentialBarIndex)->GetEnergyCalibFormula();
    ener                  = (enercalibFormula->Eval(GetQMean()));
    // std::cout << "Predicted Energy : " << ener << std::endl;
#else
    // Using single point calibration, and using equation of straight line to
get y correspoinding to a x ener = (20. / (1. *
GetPeakPos(vecOfBarsNamess[sequentialBarIndex]))) * GetQMean(); #endif return
ener;
  }
}*/
...
Double_t ScintillatorBar_F::GetQMeanCorrected() {
  ...
  ener = (enercalibFormula->Eval(GetQMean()));
  // std::cout<<"QMean Corrected "<<ener<<"keV or "<<ener/1000.0<<"
  // MeV"<<std::endl;
  //  std::cout << "Predicted Energy : " << ener << std::endl;
#else
  // std::cout << "entered else" <<std::endl;
  // Using single point calibration, and using equation of straight line to get
  // y correspoinding to a x ener = (20. / (1. *
  // GetPeakPos(vecOfBarsNamess[sequentialBarIndex]))) * GetQMean(); std::cout
  // << RED << "BarIndex : " << fBarIndex << " : Peak Pos : " <<
  // vecOfPeakPos[fBarIndex] << " : " << GetQMean() <<std::endl;
