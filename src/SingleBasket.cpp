...
ClassImp(ismran::SingleBasket)
  ...
  SingleBasket::SingleBasket(
      const SingleBasket
          &sb) { // doing deep copy to ensure the object (scintillator bar) is
                 // copied as a whole and not just the pointer to it, which will
                 // be deleted later
    for (unsigned int i = 0; i < sb.GetBasket().size(); i++) {
      fVecOfScintillators.push_back(
          new ScintillatorBar_F(*(sb.GetBasket()[i])));
    }
    BasketEnergy = sb.BasketEnergy;
    // SetBasketParameters();
    // SetBasketMeanTime();
    // SetBasketStdDevT();
    // fVecOfNeighbours = sb.fVecOfNeighbours; //here shallow copy works, as the
    // members are not objects of another class Print();
  }
  ...
  void SingleBasket::Print() {
    std::cout << RED << "******************** Size : " << size()
              << "***************************************" << RESET
              << std::endl;
    // std::vector<std::shared_ptr<ScintillatorBar_F>> scintBarVec =
    // GetBasket();
    ...
    std::cout << std::endl;
    std::cout << "Number of bars in Basket is: " << scintBarVec.size()
              << std::endl;
    std::cout << "Total Energy of Basket is: " << GetBasketEnergy()
              << std::endl;
    std::cout << "Start Time of Basket is: " << GetBasketStartTime()
              << std::endl;
    // std::cout << "Mean Time of Basket is: " << GetBasketMeanTime() <<
    // std::endl; std::cout << "StdDev in Time of Basket is: " <<
    // GetBasketStdDevT() << std::endl; std::cout << "StdDev in X of Basket is:
    // " << GetBasketStdDevX() << std::endl; std::cout << "StdDev in Y of Basket
    // is: " << GetBasketStdDevY() << std::endl; std::cout << "COM Bar of Basket
    // is: " << GetBasketCOMBar() << std::endl;
  }
  //////////////////////////////////////////////////////////////////////
  // Getters
  std::vector<ScintillatorBar_F *> SingleBasket::GetBasket() const {
  ...
  ULong64_t SingleBasket::GetBasketEndTime() {
    auto maxT = std::max_element(
        fVecOfScintillators.begin(), fVecOfScintillators.end(),
        [](const ScintillatorBar_F *a, const ScintillatorBar_F *b) {
          return a->GetTStampLarge() < b->GetTStampLarge();
        });
    return (*maxT)->GetTStampLarge();
  }
  ULong64_t SingleBasket::GetBasketDuration() {
    return GetBasketEndTime() - GetBasketStartTime();
  }
  ...
  // std::vector<ushort*> SingleBasket::GetBasketNeighbours() const {return
  // fVecOfNeighbours;}
  //////////////////////////////////////////////////////////////////////
  // Setters
  ...
  void SingleBasket::SetBasketEnergy(Double_t E) { BasketEnergy += E; }
  ...
  // 		  BasketEnergy +=
  // fVecOfScintillators[j]->GetQMeanCorrected()/1000.0;
  // 	  }
  // 	  SetBasketMeanTime();
  // 	  SetBasketStdDevT();
  // 	  COMIndex = static_cast<int>(std::floor(H2D->GetMean(1)))*10 +
  // static_cast<int>(std::floor(H2D->GetMean(2))); 	  sigX =
  // H2D->GetStdDev(1); 	  sigY = H2D->GetStdDev(2); 	  delete H2D;
  //   }
  ...
  // 			  sum += TMath::Power((meanT -
  // GetBasketEventTime(i)),2); 		  }else{ sum +=
  // TMath::Power((GetBasketEventTime(i) - meanT),2);
  // 		  }
  // 	  }
  ...
  // 	  std::unordered_set<ushort *> uniqueValues; //only unique values can be
  // entered into unordered_set 	  std::vector<ushort *> vecBarNeighbour;
  // for(int i=0; i < size(); i++){ 		  vecBarNeighbour =
  // fVecOfScintillators[i]->GetNeighbourIndx(); //get the neighbours of each
  // bar 		  for(ushort* val: vecBarNeighbour){ if
  // (uniqueValues.insert(val).second){	//enter only if unique (.second tells if
  // the number was entered. if not entered then not unique)
  ...
  // 		  }
  // 		  std::vector<ushort *> neighboursOfNewScint =
  // scint->GetNeighbourIndx(); 		  for(ushort* val:
  // neighboursOfNewScint){ 			  if
