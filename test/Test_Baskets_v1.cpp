...
int main(int argc, char *argv[]){
	...
	if(argc==6 and numVetoLayers!=0){
		for(int i=0; i<vetoedbasketVecSize-1; i++){
			...
			hTimeEnergy->Fill(log10(vecOfBasketsPostVeto[i+1]->GetBasketEnergy()), log10(vecOfBasketsPostVeto[i+1]->GetBasketStartTime()-vecOfBasketsPostVeto[i]->GetBasketEndTime()));
		}
